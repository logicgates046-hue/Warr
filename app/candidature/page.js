  ...
  const refreshTickets = async () => {
    const [{ data: wantamData }, { data: tutamData }] = await Promise.all([
      supabase
        .from('tickets')
        .select(
          `id, vote_count, president:president_id ( name, photo_url ), deputy:deputy_id ( name, photo_url )`
        )
        .eq('side', 'WANTAM'),
      supabase
        .from('tickets')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: profile }, ticketResult] = await Promise.all([
        supabase.from('profiles').select('side, candidature_ticket_id').eq('id', authData.user.id).single(),
        refreshTickets(),
      ]);
    ...
    <main className="home">
      <section className="hero">
        ...
        {!side ? (
          <>
            <p className="intro">
              You haven't voted in Battle yet. Browse both sides' tickets below — vote in
              Battle to unlock voting here.
            </p>

            {wantamTickets[wantamIndex] && (
              <>
                <TicketCard
                  ticket={wantamTickets[wantamIndex]}
                  sideKey="WANTAM"
                  locked
                  onVote={handleVote}
                />
                <button onClick={shuffleWantam} className="shuffle-button">
                  SHUFFLE WANTAM
                </button>
              </>
            )}
            ...
            {tutamTickets[tutamIndex] && (
              <>
                <TicketCard
                  ticket={tutamTickets[tutamIndex]}
                  sideKey="TUTAM"
                  locked
                  onVote={handleVote}
                />
                <button onClick={shuffleTutam} className="shuffle-button">
                  SHUFFLE TUTAM
                </button>
              </>
            )}
